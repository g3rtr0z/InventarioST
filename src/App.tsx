...
function App() {
  ...
  const handleSaveItem = async (item: ItemInventario) => {
    try {
      ...
      if (esCategoriaMonitor(item.categoria) && item.responsable.trim()) {
        const monitorDuplicado = buscarMonitorConResponsable(
          items,
          item.responsable,
          editingItem?.id
        );
        if (monitorDuplicado) {
          throw new Error(getMensajeMonitorResponsableDuplicado(item.responsable, monitorDuplicado));
        }
      }
