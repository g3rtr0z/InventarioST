...
import {
  buscarMonitorConResponsable,
  esCategoriaMonitor,
  getMensajeMonitorResponsableDuplicado
} from '../utils/monitorResponsable';
import { sedeRequiereEdificio } from '../utils/sedeFormulario';
...
export default function ItemForm({ item, categorias, sedes, items, onSave, onCancel, currentUserEmail = '', currentUserName = '', isAdmin = false, usuarios = [] }: ItemFormProps) {
  ...
  const [monitorAsociadoPopupMensaje, setMonitorAsociadoPopupMensaje] = useState<string | null>(null);
  const [monitorErrorReabrirBuscar, setMonitorErrorReabrirBuscar] = useState(false);
  const [monitorResponsableAbierto, setMonitorResponsableAbierto] = useState(false);
  const [monitorBuscarAbierto, setMonitorBuscarAbierto] = useState(false);
  const [monitorFiltroBusqueda, setMonitorFiltroBusqueda] = useState('');
  const monitorResponsableRef = useRef<HTMLDivElement>(null);
  ...
  const getSedeFormulario = (): string =>
    (formData.sede || item?.sede || '').trim();

  // Función helper para verificar si un campo es obligatorio
  const isCampoObligatorio = (nombreCampo: string): boolean => {
    if (nombreCampo === 'edificio' && !sedeRequiereEdificio(getSedeFormulario())) {
      return false;
    }
  ...
  const esEquipoVinculable = (categoria: string) => {
    const cat = categoria.trim().toLowerCase();
    return cat === 'computadora' || cat === 'laptop' || cat.includes('computador');
  };

  const equiposVinculables = items.filter((existingItem) => {
    if (existingItem.estado === 'Baja') return false;
    if (item && existingItem.id === item.id) return false;
    return esEquipoVinculable(existingItem.categoria);
  });

  const getEquipoVinculoLabel = (equipo: ItemInventario) => {
    const responsable = equipo.responsable?.trim() || 'Sin responsable';
    return `${responsable} - ${equipo.nombre}`;
  };

  const equiposVinculablesOrdenados = useMemo(() => {
    return [...equiposVinculables].sort((a, b) =>
      getEquipoVinculoLabel(a).localeCompare(getEquipoVinculoLabel(b), 'es')
    );
  }, [equiposVinculables]);

  const equiposVisiblesEnLista = useMemo(() => {
    const primerosDiez = equiposVinculablesOrdenados.slice(0, 10);
    const seleccionadoId = formData.equipoVinculadoId;
    if (seleccionadoId && !primerosDiez.some((equipo) => equipo.id === seleccionadoId)) {
      const seleccionado = equiposVinculablesOrdenados.find((equipo) => equipo.id === seleccionadoId);
      if (seleccionado) {
        return [seleccionado, ...primerosDiez.filter((equipo) => equipo.id !== seleccionadoId)].slice(0, 10);
      }
    }
    return primerosDiez;
  }, [equiposVinculablesOrdenados, formData.equipoVinculadoId]);

  const equiposFiltradosBusqueda = useMemo(() => {
    const termino = monitorFiltroBusqueda.trim().toLowerCase();
    if (!termino) return equiposVinculablesOrdenados;
    return equiposVinculablesOrdenados.filter((equipo) => {
      const etiqueta = getEquipoVinculoLabel(equipo).toLowerCase();
      return (
        etiqueta.includes(termino) ||
        equipo.responsable?.toLowerCase().includes(termino) ||
        equipo.nombre.toLowerCase().includes(termino)
      );
    });
  }, [equiposVinculablesOrdenados, monitorFiltroBusqueda]);

  const equipoVinculadoSeleccionado = useMemo(() => {
    if (formData.equipoVinculadoId) {
      return equiposVinculables.find((equipo) => equipo.id === formData.equipoVinculadoId);
    }
    if (formData.responsable?.trim()) {
      return equiposVinculables.find(
        (equipo) => equipo.responsable?.trim() === formData.responsable.trim()
      );
    }
    return undefined;
  }, [equiposVinculables, formData.equipoVinculadoId, formData.responsable]);

  const mostrarPopupMonitorAsociado = (mensaje: string, volverABuscar = true) => {
    setMonitorErrorReabrirBuscar(volverABuscar);
    setMonitorResponsableAbierto(false);
    setMonitorBuscarAbierto(false);
    setMonitorAsociadoPopupMensaje(mensaje);
  };

  const cerrarPopupMonitorAsociado = () => {
    const volverABuscar = monitorErrorReabrirBuscar;
    setMonitorAsociadoPopupMensaje(null);
    setMonitorErrorReabrirBuscar(false);
    if (volverABuscar) {
      setMonitorBuscarAbierto(true);
    }
  };

  const handleSelectEquipoMonitor = (equipoId: string) => {
    const equipo = equiposVinculables.find((e) => e.id === equipoId);
    if (!equipo) return;

    const responsable = equipo.responsable?.trim() || '';
    const monitorExistente = buscarMonitorConResponsable(items, responsable, item?.id);
    if (monitorExistente && responsable) {
      mostrarPopupMonitorAsociado(
        getMensajeMonitorResponsableDuplicado(responsable, monitorExistente),
        true
      );
      return;
    }
    ...
    setMonitorResponsableAbierto(false);
    setMonitorBuscarAbierto(false);
    setMonitorFiltroBusqueda('');
  };

  const limpiarResponsableMonitor = () => {
    setFormData((prev) => ({ ...prev, responsable: '', equipoVinculadoId: '' }));
    setMonitorAsociadoPopupMensaje(null);
    setMonitorErrorReabrirBuscar(false);
    setMonitorResponsableAbierto(false);
  };
  ...
  useEffect(() => {
    if (!monitorResponsableAbierto) return;
    const handleClickFuera = (event: MouseEvent) => {
      if (monitorResponsableRef.current && !monitorResponsableRef.current.contains(event.target as Node)) {
        setMonitorResponsableAbierto(false);
      }
    };
    document.addEventListener('mousedown', handleClickFuera);
    return () => document.removeEventListener('mousedown', handleClickFuera);
  }, [monitorResponsableAbierto]);

  useEffect(() => {
    if (!monitorBuscarAbierto) {
      setMonitorFiltroBusqueda('');
    }
  }, [monitorBuscarAbierto]);
  ...
  useEffect(() => {
    if (item) {
      ...
      const sedeItem = (rest.sede || (sedes.length > 0 ? sedes[0] : '')).trim();
      const nuevoFormData: any = {
        ...
        piso: rest.piso || '',
        edificio: sedeRequiereEdificio(sedeItem) ? (rest.edificio || '') : '',
        sede: sedeItem,
        ...
        equipoVinculadoId: rest.equipoVinculadoId || '',
      };
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    // Cesfam Barroso no usa edificio
    if (name === 'sede' && !sedeRequiereEdificio(value)) {
    ...
    // Si deja de ser Monitor, quitar vínculo con equipo y cerrar selectores
    if (name === 'categoria' && !esCategoriaMonitor(value)) {
      setFormData(prev => ({ ...prev, equipoVinculadoId: '' }));
      setMonitorAsociadoPopupMensaje(null);
      setMonitorErrorReabrirBuscar(false);
      setMonitorResponsableAbierto(false);
      setMonitorBuscarAbierto(false);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const sanitizedData = {
      ...
      equipoVinculadoId: formData.equipoVinculadoId ? sanitizeText(formData.equipoVinculadoId) : undefined
    };

    if (!sedeRequiereEdificio(sanitizedData.sede)) {
      sanitizedData.edificio = '';
    }
    ...
    // Un solo monitor por responsable
    if (esCategoriaMonitor(sanitizedData.categoria) && sanitizedData.responsable.trim()) {
      const monitorExistente = buscarMonitorConResponsable(items, sanitizedData.responsable, item?.id);
      if (monitorExistente) {
        mostrarPopupMonitorAsociado(
          getMensajeMonitorResponsableDuplicado(sanitizedData.responsable, monitorExistente),
          true
        );
        return;
      }
  ...
  return (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {camposSeccion.map((campoConfig: CampoFormulario) => {
                        ...
                        if (campoConfig.nombre === 'edificio') {
                          if (!sedeRequiereEdificio(getSedeFormulario())) {
                            return null;
                          }
                          return (
                            <div key={campoConfig.nombre}>
                            ...
                            <>
                              <div key={campoConfig.nombre} className="md:col-span-2">
                                <label htmlFor="responsable" className="block mb-1 text-[11px] font-bold text-slate-500 uppercase tracking-wide">
                                  ...
                                  {esMonitor && (
                                    <span className="text-gray-500 text-xs ml-2 font-normal normal-case">
                                      (desde Computadora o Laptop)
                                    </span>
                                  )}
                                </label>

                                {esMonitor ? (
                                  <div ref={monitorResponsableRef} className="relative">
                                    <input
                                      type="hidden"
                                      id="responsable"
                                      name="responsable"
                                    ...
                                    <button
                                      type="button"
                                      id="responsable-monitor-trigger"
                                      onClick={() => setMonitorResponsableAbierto((prev) => !prev)}
                                      className={`w-full px-3 py-2 border border-gray-300 focus:outline-none focus:${INSTITUTIONAL_COLORS.borderPrimary} rounded-xl bg-white text-left flex items-center justify-between gap-2`}
                                    >
                                      <span className={equipoVinculadoSeleccionado ? 'text-slate-800' : 'text-slate-400'}>
                                        {equipoVinculadoSeleccionado
                                          ? getEquipoVinculoLabel(equipoVinculadoSeleccionado)
                                          : 'Seleccione responsable y equipo'}
                                      </span>
                                    ...
                                    {monitorResponsableAbierto && (
                                      <div className="absolute z-20 mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden">
                                        {equiposVinculablesOrdenados.length > 0 ? (
                                          <>
                                            <ul className="max-h-56 overflow-y-auto divide-y divide-slate-100">
                                              {equiposVisiblesEnLista.map((equipo) => (
                                                <li key={equipo.id}>
                                                  <button
                                                    type="button"
                                                    onClick={() => handleSelectEquipoMonitor(equipo.id)}
                                                    className={`w-full px-3 py-2.5 text-left text-sm hover:bg-green-50 transition-colors ${
                                                      formData.equipoVinculadoId === equipo.id ? 'bg-green-50 font-semibold text-green-900' : 'text-slate-700'
                                                    }`}
                                                  >
                                                    {getEquipoVinculoLabel(equipo)}
                                                  </button>
                                                </li>
                                              ))}
                                            </ul>
                                            <div className="flex items-center justify-between gap-2 px-3 py-2 border-t border-slate-100 bg-slate-50">
                                              {formData.equipoVinculadoId || formData.responsable ? (
                                                <button
                                                  type="button"
                                                  onClick={limpiarResponsableMonitor}
                                                ...
                                                <span className="text-[10px] text-slate-400">
                                                  Mostrando {equiposVisiblesEnLista.length} de {equiposVinculablesOrdenados.length}
                                                </span>
                                              )}
                                              <button
                                                type="button"
                                                ...
                                                className={`px-3 py-1.5 text-xs font-semibold text-white ${INSTITUTIONAL_COLORS.bgPrimary} hover:bg-green-900 rounded-lg transition-colors`}
                                              >
                                                Buscar
                                              </button>
                                            </div>
                                          ...
                                          <p className="px-3 py-3 text-sm text-slate-500">
                                            No hay computadoras ni laptops registradas.
                                          </p>
                                        )}
                                      </div>
                                    )}
                                    ...
                                    {monitorBuscarAbierto && (
                                      <div
                                        ...
                                        onClick={() => setMonitorBuscarAbierto(false)}
                                      >
                                        <div
                                        ...
                                        >
                                          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
                                            <h3 className="text-sm font-bold text-slate-800">Buscar responsable y equipo</h3>
                                            <button
                                              type="button"
                                              onClick={() => setMonitorBuscarAbierto(false)}
                                          ...
                                          <div className="p-4">
                                            <input
                                              type="text"
                                              value={monitorFiltroBusqueda}
                                              onChange={(e) => setMonitorFiltroBusqueda(e.target.value)}
                                              placeholder="Filtrar por responsable o equipo..."
                                              autoFocus
                                              className={`w-full px-3 py-2 border border-gray-300 focus:outline-none focus:${INSTITUTIONAL_COLORS.borderPrimary} rounded-xl`}
                                            />
                                            <ul className="mt-3 max-h-64 overflow-y-auto border border-slate-100 rounded-xl divide-y divide-slate-100">
                                              {equiposFiltradosBusqueda.length > 0 ? (
                                                equiposFiltradosBusqueda.map((equipo) => (
                                                  <li key={equipo.id}>
                                                    <button
                                                      type="button"
                                                      onClick={() => handleSelectEquipoMonitor(equipo.id)}
                                    ...
                                    <p className="mt-1 text-xs text-gray-500">
                                      Formato: Responsable - Equipo. Se muestran hasta 10. Use Buscar para ver y filtrar el listado completo.
                                    </p>
                                  </div>
                                ) : (
                                  <input
    ...
    {monitorAsociadoPopupMensaje && (
      <div
        className="fixed inset-0 z-[80] flex items-center justify-center bg-black/50 p-4"
        onClick={cerrarPopupMonitorAsociado}
      >
        <div
          className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden"
          onClick={(e) => e.stopPropagation()}
          role="alertdialog"
          aria-labelledby="monitor-asociado-titulo"
          aria-describedby="monitor-asociado-mensaje"
        >
          <div className="px-5 py-4 border-b border-slate-100">
            <h3 id="monitor-asociado-titulo" className="text-base font-bold text-slate-800">
              Monitor ya asociado
            </h3>
          </div>
          <div className="px-5 py-4">
            <p id="monitor-asociado-mensaje" className="text-sm text-slate-600 leading-relaxed">
              {monitorAsociadoPopupMensaje}
            </p>
          </div>
          <div className="px-5 py-4 border-t border-slate-100 flex justify-end">
            <button
              type="button"
              onClick={cerrarPopupMonitorAsociado}
              className={`px-4 py-2 text-sm font-semibold text-white ${INSTITUTIONAL_COLORS.bgPrimary} hover:bg-green-900 rounded-xl transition-colors`}
            >
              Entendido
            </button>
          </div>
